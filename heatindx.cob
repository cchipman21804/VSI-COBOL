      ...
      * Calculation fields.
       ...
       01 absval                               pic s999v9 value zero
      ...
      * Display fields.
       ...
       procedure division.
       100-main-para.
           perform 110-display-title-screen thru 150-display-results.
       ...
       120-outside-temp-data-entry.
           ...
           move function LOWER-CASE (outside-temp-in) 
                 TO outside-temp-in 

           if outside-temp-in IS EQUAL TO "zero"
           then go to 999-end-program
           else
              compute outside-temp = function numval(outside-temp-in)
           end-if

           if outside-temp IS EQUAL to ZERO
           then
           ...
           if outside-temp > max-temp
              ...
              go to 120-outside-temp-data-entry
           end-if
       ...
       130-rel-humidity-data-entry.
           display "Enter relative humidity as % " with no advancing
           display "(nnn.n): " with no advancing
           accept relative-humidity-in
           move function LOWER-CASE (relative-humidity-in)
                 TO relative-humidity-in 

           if relative-humidity-in IS EQUAL TO "zero"
            then go to 999-end-program
           else
           ...
           if relative-humidity IS EQUAL TO ZERO
           then
              go to 999-end-program
           end-if
           ...
           if relative-humidity > max-rh
              ...
              display spaces
              go to 130-rel-humidity-data-entry
           end-if
      ...
      *     display "Average of heat index & outside temp: " heat-index
           ...
           when outside-temp IS GREATER THAN OR EQUAL TO 80 AND
              ...
              perform 141-absolute-value

              compute adjustment rounded =
                      ((13 - relative-humidity) / 4) *
                      function sqrt((17 - absval) / 17)
      ...
      * by Steadman.
       ...
       141-absolute-value.
      *
      * Substitute for function abs()
           subtract 95 from outside-temp giving absval
           if absval is less than ZERO 
              then compute absval = 95 - outside-temp
           end-if. 
