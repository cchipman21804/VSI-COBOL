      ...
      * Specify playing field dimensions here:
       01 max-x        pic 999 value 70.
      *
      * max-x must be the same value as c occurences until "depending 
      * on" bug is fixed.
      *
      * 75 columns is the maximum that will fit on an 80 col display.
      * 
       01 max-y        pic 99 value 20.
      *
      * max-y must be the same as r occurences until "depending on" bug
      * is fixed.
      *
      ...
      *
      * These variables will substitute for the sign function.
       01 x-sign-function pic s9 sign is leading separate.
       01 y-sign-function pic s9 sign is leading separate.
      *
      ...
      *                       Future features:
      *
      * Keep a running tally high score file with fields for
      * initials, last game stats (datetime string @start, playerscore,
      * robotscore, # turns4, # teleports, # pursuers, max-x, max-y)
      *
       PROCEDURE DIVISION.
       100-primary.
           perform 110-display-title-screen THRU 
      ...
      * Comment out the next two display lines after debugging
           ...
           compute random-num = function random(hund-sec)
      ...
      * 300-exit-termination SECTION.
       ...
       110-display-title-screen.
           ...
           display spaces
           display "Ported to VSI COBOL for OpenVMS "
                    with no advancing
           display "by: Cliff Chipman, Salisbury, MD"
           display "August 18, 2020"
           display spaces
       ...
       120-instructions-prompt.
           ...
           END-EVALUATE.
      *
      * Comment out this paragraph if you want a fixed number of 
      * pursuers at all times:
      * 130-pursuers-prompt.
      ...
      *                                                               *
       430-teleport-character.
           ...
           if c (y, x) IS NOT EQUAL TO emptyspace
           OR x < 2 OR x > max-x - 1 
           OR y < 2 OR y > max-y - 1 then
              go to 430-teleport-character
           end-if
      *
      * Comment out these display lines after debugging
           display "TELEPORTING TO:"
           display "N: " n " |X: " x " |Y: " y.
      *                                                               *
      * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      ...
      * Calculate player's direction and move robot toward player
           perform 451-sign-function
           compute pursuerx(n) = pursuerx(n) + x-sign-function 
           compute pursuery(n) = pursuery(n) + y-sign-function
      *
      ...
      *
       451-sign-function.
      *
      * This will serve as a substitute for the sign function
      ...
      *                            function sign(playery - pursuery(n))
           EVALUATE true
               WHEN (playerx - pursuerx(n)) IS LESS THAN ZERO 
                    move -1 to x-sign-function

               WHEN (playerx - pursuerx(n)) IS GREATER THAN ZERO 
                    move 1 to x-sign-function

               WHEN (playerx - pursuerx(n)) IS EQUAL TO ZERO
                    move 0 to x-sign-function
           
           END-EVALUATE
           
           EVALUATE true
               WHEN (playery - pursuery(n)) IS LESS THAN ZERO 
                    move -1 to y-sign-function 

               WHEN (playery - pursuery(n)) IS GREATER THAN ZERO 
                    move 1 to y-sign-function

               WHEN (playery - pursuery(n)) IS EQUAL TO ZERO 
                    MOVE 0 to y-sign-function 
